<template>
<div style="background: linear-gradient(to bottom,  white, rgba(190, 225, 230, 0.5), rgba(135, 206, 250, 0.5)), rgba(29, 63, 85, 0.5);">
  <div>
    <h1>정기예금 상품 목록</h1>
    <hr>
    <p></p>
    ...
    <div v-if="filteredDeposits.length" class="cards-container">
      <div class="card" v-for="deposit in filteredDeposits" :key="deposit.id">
        <img :src="getImage(deposit.kor_co_nm)" :alt="deposit.name" class="bank-image" />
        <div class="card-content">
          <h2 class="product-name">{{ deposit.kor_co_nm }}</h2>
          <p class="highlight">상품 이름: {{ deposit.fin_prdt_nm }}</p>
          <p>옵션 개수: {{ deposit.deposit_options.length }}개</p>
          <p>최소: {{ getMinTerm(deposit.deposit_options) }}개월</p>
          <p>최대: {{ getMaxTerm(deposit.deposit_options) }}개월</p>
          <p class="highlight">최고 우대금리: {{ getMaxBenefitRate(deposit.deposit_options) }}%</p>
          <button @click="goToDetail(deposit.fin_prdt_cd, 'deposit')" class="detail-button">상세 정보 바로가기</button>
        </div>
      </div>
    </div>
    <div v-else>
      <p>예금 상품을 불러오는 중입니다...</p>
    </div>
  </div>
</div>
</template>
...
const fetchDeposits = async () => {
  axios({
    method: 'get',
    url: `${store.baseURL}/deposits/products/`,
    headers: {
...
const sortedDeposits = computed(() => {
  return deposits.value.slice().sort((a, b) => getMaxBenefitRate(b.deposit_options) - getMaxBenefitRate(a.deposit_options))
})

const filteredDeposits = computed(() => {
  return sortedDeposits.value.filter(product => {
    const matchBank = selectedBank.value === '' || product.kor_co_nm === selectedBank.value
    const matchMaxTerm = selectedMaxTerm.value === '' || product.deposit_options.some(option => option.save_trm <= parseInt(selectedMaxTerm.value))
